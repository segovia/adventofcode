...
public class D04_GiantSquid_2 {

    @Test
    public void test() throws IOException {
        List<String> fileInputs = Utils.getInputsFromFiles(this.getClass());
        assertThat(run(fileInputs.get(0)), is(1924L));
        assertThat(run(fileInputs.get(1)), is(18063L));
    }
    ...
    private long run(String input) {
        ...
        boolean[] boardWon = new boolean[boards.length];
        int boardWonCount = 0;
                    ...
                    for (int j = 0; j < 5; j++) {
                        if (num == boards[b][i][j] && !marked[b][i][j]) {
                            ...
                            if (isWin) {
                                boardWon[b] = true;
                                boardWonCount++;
                                if (boardWonCount == boards.length) {
                                    winningBoard = b;
                                    winningNum = num;
                                    break numLoop;
                                }
